...
if (!inputPath) throw new Error("Pass the edited Crystal Ball JSON file path.");

const projectRoot = path.resolve(__dirname, "..");
const contentPath = path.join(projectRoot, "assets/data/crystal-ball-predictions-ru.json");
const incoming = JSON.parse(fs.readFileSync(path.resolve(inputPath), "utf8"));
const content = JSON.parse(fs.readFileSync(contentPath, "utf8"));

if (incoming.ritual !== "crystal-ball" || !Array.isArray(incoming.entries)) {
  throw new Error("The edited file is not a Crystal Ball phrase export.");
}
...
console.log(`Imported ${content.entries.length} Crystal Ball phrases.`);
